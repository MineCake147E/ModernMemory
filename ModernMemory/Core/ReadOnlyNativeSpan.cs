...
namespace ModernMemory
{
    ...
    public readonly ref struct ReadOnlyNativeSpan<T>
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        public ReadOnlyNativeSpan<T> Slice(nuint start) => TrySlice(out var span, start) ? span : ThrowSliceExceptions(start);

        /// <inheritdoc cref="ReadOnlySpan{T}.Slice(int, int)"/>
        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        public ReadOnlyNativeSpan<T> Slice(nuint start, nuint length) => TrySlice(out var span, start, length) ? span : ThrowSliceExceptions(start, length);
        ...
        [DoesNotReturn]
        private ReadOnlyNativeSpan<T> ThrowSliceExceptions(nuint start)
        {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        [DoesNotReturn]
        private ReadOnlyNativeSpan<T> ThrowSliceExceptions(nuint start, nuint length)
        {
            ArgumentOutOfRangeException.ThrowIfGreaterThan(start, Length);
        ...
        public nuint CopyAtMostTo(NativeSpan<T> destination, nuint srcOffset = 0)
        {
            var len = Length;
            nuint copyLength = 0;
            if (srcOffset < len && !destination.IsEmpty)
            {
                ref readonly var srcHead = ref ElementAtUnchecked(srcOffset);
                copyLength = nuint.Min(len - srcOffset, destination.Length);
        ...
        /// <summary>
        /// Indicates whether the owner of two specified <see cref="ReadOnlyNativeSpan{T}"/> objects are equal.
        /// </summary>
        ...
        /// <summary>
        /// Indicates whether the owner of two specified <see cref="ReadOnlyNativeSpan{T}"/> objects are not equal.
        /// </summary>
        /// <param name="left">The first <see cref="ReadOnlyNativeSpan{T}"/> to compare.</param>
        /// <param name="right">The second  <see cref="ReadOnlyNativeSpan{T}"/> to compare.</param>
        /// <returns>
