...
namespace ModernMemory.DataFlow
{
    public ref struct DataWriter<T, TBufferWriter> where TBufferWriter : IBufferWriter<T>
    {
        ...
        private readonly bool HasBufferWriter
        {
            ...
            get => !(!RuntimeHelpers.IsReferenceOrContainsReferences<TBufferWriter>() && default(TBufferWriter) is DataWriter.DummyBufferWriter<T>) && !Unsafe.IsNullRef(ref writer);
        }

        public readonly bool IsLengthConstrained => !RuntimeHelpers.IsReferenceOrContainsReferences<TBufferWriter>() && default(TBufferWriter) is DataWriter.DummyBufferWriter<T> || flags == 0;
    ...
    public static class DataWriter
    {
        ...
        public static DataWriter<T, DummyBufferWriter<T>> CreateFrom<T>(T[] array) => new(array);
